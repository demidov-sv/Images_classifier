    ...
    try:
        ...
        while True:
            ...
            if r2.status_code >= 400:
                print(f"ОШИБКА: Сервер вернул код {r2.status_code} для задачи {task_id[:8]}...")
            ...
            if "результат" in body:
                ...
                objects = re.findall(r'<td>\s*(.*?)\s*</td>\s*<td[^>]*>\s*([\d\.]+%)\s*</td>', r2.text)
                
                if objects:
                    objects_str = ", ".join([f"{name} ({conf})" for name, conf in objects])
                    print(f"УСПЕХ [{task_id[:8]}...]: {elapsed:.1f}с (модель: {proc_time}) | Объекты: {objects_str}")
                else:
                    print(f"УСПЕХ [{task_id[:8]}...]: {elapsed:.1f}с (модель: {proc_time}) | Объектов не найдено")
...
async def main():
    ...
    async with httpx.AsyncClient(limits=limits, timeout=30.0, follow_redirects=True) as client:
        ...
        print(f"Запуск теста: {RPS} RPS в течение {DURATION} сек (Всего запросов: {total_requests})")
        print("--------------------------------------------------------------------------------")
        ...
        for status in ("success", "error", "timeout", "exception"):
            ...
            if count:
                ...
                print(f" {status.upper()}: {count} шт. (среднее время полного цикла: {avg:.2f}s)")
            else:
        ...
        if times_by_status["success"]:
            ...
            for p in (50, 90, 95, 99):
                ...
                print(f"  P{p}: {val:.2f} сек (столько или быстрее выполнилось {p}% запросов)")
